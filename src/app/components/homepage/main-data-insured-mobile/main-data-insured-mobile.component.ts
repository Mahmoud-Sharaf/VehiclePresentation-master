...
@Component({
  selector: "app-main-data-insured-mobile",
  templateUrl: "./main-data-insured-mobile.component.html",
  styleUrls: ["./main-data-insured-mobile.component.css"],
})
export class MainDataInsuredMobileComponent implements OnInit {
  currentYear: number = new Date().getFullYear();
  formdata: FormGroup;
  CaptchaFormMobile: FormGroup;
  notValidHere: any[] = [];
  ...
  ngOnInit() {
    ...
    this.CaptchaFormMobile = this.fb.group({
      ...
      this.http
        ...
        .subscribe(
          (res: any) => {
            ...
            let drivers = responseModel.drivers;
            drivers.splice(0, 1);
            this.data.setEditDrivers(drivers)
  ...
  onClickSubmit(data) {
    ...
    let myStorage = window.localStorage;
    let captchaInput = myStorage.getItem("captchaInputImgMobile");
    let captchaTOken = myStorage.getItem("captchaInputTOKENMobile");
    // suequence
    if (this.sequenceOnly == true && this.vehicleOwnerId == false) {
      if (this.formdata.invalid || this.CaptchaFormMobile.invalid) {
        this.loading = false;
